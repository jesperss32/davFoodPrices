...
def overlap_in_markets(df1, df2):
    df1_markets = df1.market_ID.unique()
    df2_markets = df2.market_ID.unique()
    overlap = [m for m in df1_markets if m in df2_markets]
    return sorted(overlap)
...
def get_data_selection(df, markets=None, countries=None, years=None, products=None):
    if countries:
    ...
    if markets:
        df = df.loc[df['market_ID'].isin(markets)]
    return df
